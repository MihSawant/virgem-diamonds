...
const Top_Rated_Seller = ({ initialValues }) => {
        ...
        <div className='!h-auto'>
            <div className='bg-white border h-full border-gray-400/20 p-4 group'>
                <div className='flex items-center justify-between mb-3'>
                    <span className='bg-violet-900 px-4 py-0.5 sm:text-sm text-[12px] text-white font-montserrat font-[500]'>
                        {initialValues.sale_badge}
                    </span>
                ...
                <Link href={initialValues.pro_link}>
                    <Image src={initialValues.img} alt={initialValues.alt} title={initialValues.alt} width={240} height={240} className='w-full mx-auto object-contain min-h-auto h-40 sm:my-6 transition-all group-hover:scale-110' />
                </Link>
                {get_badge(initialValues.stock_badge)}
                <Link href={initialValues.pro_link} className='sm:text-[17px] text-sm text-black group-hover:text-violet-900 font-[600] text-center font-montserrat capitalize sm:mb-3 mb-2 block ' >
                    {initialValues.title}
                </Link>
                <div className='relative overflow-hidden'>
                    <p className='sm:text-lg text-md text-black uppercase font-montserrat font-[500] text-center group-hover:opacity-0'>
                        ${initialValues.price}
                    </p>
                    {
                        productInCart ?
                            ...
                            <span onClick={() => AddToCart()} className='cursor-pointer text-sm text-black hover:text-violet-900 uppercase font-montserrat font-[600] inline-flex mx-auto rounded items-center group gap-2 px-6 absolute group-hover:bottom-0 -bottom-full transition-all left-1/2 -translate-x-2/4 min-w-max' >
                                <Image src={'https://webby-production.s3.amazonaws.com/Starry-Heavens/Header/cart-bag.svg'} alt='cart' title='cart' className=' ' width={20} height={20} />
                                Add to cart
                            </span>
                    }
                </div>
...
export default Top_Rated_Seller
